...
export function GuestsPage() {
  ...
  // Only load guests when authenticated (the useGuests hook auto-loads on mount,
  // so we need to reload once we're sure we're authenticated)
  useEffect(() => {
    if (auth.isAuthenticated && auth.token && guests.guests.length === 0 && !guests.isLoading) {
      guests.reloadGuests();
    }
